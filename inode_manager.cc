 ...
 * Return its inum. */
uint32_t
inode_manager::alloc_inode(uint32_t type,  unsigned long long mode, unsigned short uid, unsigned short gid)
{
  /*
   * your lab1 code goes here.
...
//      }
      ...
      printf("im create:%d type%d,size%d,mode%d,uid%d,gid%d\n",inum,ino->type,ino->size,ino->mode,ino->uid,ino->gid);
      return inum;
    }
  }
...
//  }
  ...
  a.uid = ino->uid;
  a.gid = ino->gid;
  printf("im getattr: %d type%d,size%d,mode%d,uid%d,gid%d\n",inum,ino->type,ino->size,ino->mode,ino->uid,ino->gid);
}

void
inode_manager::setattr(uint32_t inum, unsigned long long mode, unsigned short uid, unsigned short gid)
{
  /*
   * your lab1 code goes here.
   * note: get the attributes of inode inum.
...
}//for lab6 simply delete all and redo
