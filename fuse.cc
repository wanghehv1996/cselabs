...
fuseserver_symlink(fuse_req_t req, const char* link, fuse_ino_t parent, const char* name){
	...
	struct fuse_entry_param entinfo;
	entinfo.attr_timeout = 0.0;
	entinfo.entry_timeout = 0.0;
	entinfo.generation = 0;
	
	r = yfs->symlink(link, parent, name, ino_num);
 	if (r != yfs_client::OK) {
		if (r == yfs_client::NOENT) {
			fuse_reply_err(req, ENOENT);
			return;
	...
	entinfo.ino = ino_num;
	getattr(ino_num, entinfo.attr);
	fuse_reply_entry(req, &entinfo);
}
...
int find_version(){
    ...
    ifstream infile;
    infile.open("yfs_client1.log");
    string sline;
    while(getline(infile, sline)){
	if(sline == "commit a new version") version_number++;
	if(sline == "to-previous version") version_number--;
	if(sline == "to-next version") version_number++;
    }
...
void sig_handler(int no) {
	...
	int version_number = find_version();
        printf("now version environment:%d\n",version_number);
	break;
    }
    case SIGUSR1:
    {
	...
	string sline;
        int v_number = find_version();
	int version_number = 0;
        ifstream infile;
        infile.open("yfs_client1.log");
	while(getline(infile, sline)){
	    if(sline == "commit a new version") version_number++;
	    if(version_number == v_number)
		break;
        }
	while(getline(infile, sline)){
	if(sline == "commit a new version" || sline == "to-previous version" || sline == "to-next version") break;
	    string label_word;
	    istringstream sin(sline);
	    sin >> label_word;
	    if(label_word == "create_file!!!" || 
	       label_word == "create_directory!!!" || 
	       label_word == "create_slink!!!" || 
	       label_word == "unlink!!!")
	         undo_list.push_back(sline);		
	}

	for(int i=undo_list.size()-1;i>=0;i--){
	    string label_word;
	    istringstream sin(undo_list[i]);
	    sin >> label_word;
	    if(label_word == "create_file!!!" || label_word == "create_directory!!!" || label_word == "create_slink!!!"){
		string name,temp;
		int parent;
		sin >> temp;
	        sin >> parent;
		...
		sin >> size;		
		string ss = "data: ";
	        int pos = undo_list[i].find(ss);		
		string idata = undo_list[i].substr(pos+6,size);
		yfs_client::inum ino_out;
		yfs->create(parent,name.c_str(),0,ino_out);
		size_t bytes_written;
		yfs->write(ino_out,idata.size(),0,idata.c_str(),bytes_written);
	    }
	}	
    ...
    case SIGUSR2:
    {
	vector<string>redo_list;
	string sline;
        int v_number = find_version() + 1;
	int version_number = 0;
        ifstream infile;
        infile.open("yfs_client1.log");
	while(getline(infile, sline)){
	...
	if(sline == "commit a new version" || sline == "to-previous version" || sline == "to-next version") break;
	    ...
	    if(label_word == "create_file!!!" || 
	       ...
	       label_word == "create_slink!!!" || 
	       label_word == "unlink!!!" ||
	       label_word == "write_to_file")
	         redo_list.push_back(sline);		
	}

	for(unsigned int i=0;i<redo_list.size();i++){
	    string label_word;
	    istringstream sin(redo_list[i]);
	    sin >> label_word;
	    if(label_word == "create_file!!!" || label_word == "create_directory!!!" || label_word == "create_slink!!!"){
		string name,temp;
		int parent;
		...
		if(label_word == "create_file!!!"){
		    yfs->create(parent,name.c_str(),0,ino_out);
		    //cout << " name inode :  " << name << ino_out;
		}
		if(label_word == "create_directory!!!")
		    yfs->mkdir(parent,name.c_str(),0,ino_out);
	    }
	    else if(label_word == "unlink!!!"){
		string name,temp;
		int parent;
	    ...
	    else if(label_word == "write_to_file"){
		string temp,data;
		yfs_client::inum ino;
		int off;
		size_t size;
		...
		sin >> temp;
	        sin >> size;
		string ss = "data: ";
	        int pos = redo_list[i].find(ss);		
		data = redo_list[i].substr(pos+6,size);
		//find the real inode!!
		size_t bytes_written;
		...
		string ssline;
        	ifstream iinfile;
		//infile.close();
		iinfile.open("yfs_client1.log");
		string label1;
		string name;
		yfs_client::inum parent,now_ino,real_ino;
		while(getline(iinfile,ssline)){
		    if(ssline == redo_list[i])break;
	    	    istringstream ssin(ssline);
	    	    ssin >> label1;
		    if(label1 == "create_file!!!"){
			ssin >> temp >> parent >> temp >> name >> temp >> now_ino;
			if(now_ino == ino){
			    bool found;
			    yfs->lookup(parent,name.c_str(),found, real_ino);
			}
		    }
		}
		cout << data.c_str() << "\n";
		yfs->write(real_ino,size,off,data.c_str(),bytes_written);
	    }
	}	
